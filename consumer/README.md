...
> 注意：消费端域名（`*.cls.tencentcs.com`）与云 API 域名（`cls.<region>.tencentcloudapi.com`）不同。`Endpoint` 字段只填写消费域名；云 API 域名由 SDK 内部根据 `Region` 自动拼接，无需手动配置。
...
下面示例与 `consumer/demo/consumer_demo.go` 完全对齐：构造 `ConsumerOption` → `NewConsumerWorkerFromOption` → `Run(ctx)` → 监听信号 → `cancel()` → 等待退出。

> `NewConsumerWorker` 已废弃（会忽略底层云 API 客户端构造错误，AccessKeyID/AccessKey 为空时返回 nil），新代码请使用返回 error 的 `NewConsumerWorkerFromOption`。
...
| `Region` | string | ✅ | 云 API 地域，写入 `X-TC-Region` 公共请求头，必须填写，否则服务端会报 `MissingParameter: Region`。 |
...
| `Query` | string | | （可选）DSL 预过滤表达式，服务端按表达式过滤后再返回日志，可显著节省网络与客户端 CPU。空字符串表示不启用。详见下方「DSL 预过滤（Query）」一节，以及官方文档 [日志消费 DSL 过滤语法](https://cloud.tencent.com/document/product/614/37908)。 |

---

## 七、DSL 预过滤（Query）

通过 `ConsumerOption.Query` 可以让服务端在返回日志前就按 DSL 表达式过滤，只把命中的日志投递给消费者：

- **降低带宽**：未命中日志根本不会下发；
- **降低 CPU**：客户端不用再做一遍解析 + 判断；
- **不改变 offset 语义**：服务端按 offset 顺序扫描，被过滤掉的日志同样会推进 `NextOffset`，断点续传不受影响。
...
留空（或不设置）即关闭服务端预过滤，行为与改动前完全一致。

### 2. 表达式语义

上面的示例含义为：**仅消费 `status` 字段大于 400 且 `message` 字段包含 `"access failed"` 关键字的日志**。

常用算子（完整列表见官方文档）：

| 算子 | 含义 |
|---|---|
| `log_keep(expr)` | 顶层包装：表达式为 true 时保留该条日志 |
| `v("field")` | 取字段值 |
| `op_and(a, b, ...)` / `op_or(...)` / `op_not(a)` | 逻辑运算 |
| `op_gt / op_ge / op_lt / op_le / op_eq / op_ne` | 数值比较 |
| `str_exist(v("field"), "kw")` | 字段值包含子串 |
| `str_eq / str_ne / str_starts_with / str_ends_with` | 字符串匹配 |
| `regex(v("field"), "pattern")` | 正则匹配 |

### 3. 更多示例

```go
// 只消费 ERROR 级别日志
Query: `log_keep(str_eq(v("level"), "ERROR"))`

// 只消费 HTTP 5xx
Query: `log_keep(op_and(op_ge(v("status"), 500), op_lt(v("status"), 600)))`

// 排除健康检查请求
Query: `log_keep(op_not(str_eq(v("path"), "/healthz")))`

// 满足任意一个条件
Query: `log_keep(op_or(str_eq(v("level"), "ERROR"), str_eq(v("level"), "FATAL")))`
```

### 4. 注意事项

- 表达式语法错误会导致 `PullLogs` 直接报错，建议在腾讯云控制台或 [文档](https://cloud.tencent.com/document/product/614/37908) 中先验证再上线。
- `Query` 是 **拉取期过滤**，不是检索分析语法（SQL/Lucene），不要把控制台检索框中的语句直接贴过来。
- `Query` 与 `StartTime / EndTime / OffsetStartTime / OffsetEndTime` 互不冲突，可同时配置。

---

## 八、Processor 接口与 ProcessorBase
...
worker, err := consumer.NewConsumerWorkerFromOption(opt, adaptor)
```
...
4. **优雅退出**：先 `cancel()` 让分区 worker 正常 flush，再 `worker.Shutdown()`；预留至少 5~10s 关闭窗口。
5. **不要每次进程退出都 `DeleteConsumerGroup`**：消费组是跨实例共享的状态，删了会清掉所有 offset。
6. **多实例横向扩容**：同 `ConsumerGroup` + 不同 `ConsumerName` 启动多个实例，服务端会按 `PartitionStrategy=2` 自动均衡分区。
7. **OffsetEndTime 一次性任务**：配置后所有分区追上 + 宽限期后自动退出，无需自己判断。
...
A：`processData` 内有 `defer recover()`，仅记录日志，offset 仍会推进。如需"失败不推进"，请见「错误处理与重试」一节。
