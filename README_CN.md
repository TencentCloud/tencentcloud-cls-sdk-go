...
最简使用方式：实现 `Processor` 接口 → 构造 `ConsumerOption` → `consumer.NewConsumerWorkerWithOption(option, processor)`（返回 error，需判断后再 `Run(ctx)`）。

> `consumer.NewConsumerWorker` 已废弃：它会忽略底层云 API 客户端的构造错误（例如 AccessKeyID/AccessKey 为空），请改用 `consumer.NewConsumerWorkerWithOption`。同理，`cls.NewYunApiLogClient` / `NewYunApiLogClientWithConfig` / `NewYunApiLogClientSimple` 已废弃，请改用 `cls.NewYunApiLogClientFromConfig`。
